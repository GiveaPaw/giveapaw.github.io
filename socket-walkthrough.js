...
scene.add(importRoot, planeRoot, interfaceRoot);

const layers = {
    solidAnimal: {
        label: 'Solid animal', color: '#496b94', group: new THREE.Group(), visible: true,
        file: 'billie-solid-animal.glb', type: 'glb', opacity: 0.28,
    },
    attachment: {
        label: 'Attachment mesh', color: '#169c8c', group: new THREE.Group(), visible: true,
        file: 'billie-attachment-surface.glb', type: 'glb', opacity: 0.88,
    },
    mechanical: {
        label: 'Mechanical interface', color: '#df7544', group: new THREE.Group(), visible: true,
        file: 'billie-mechanical-interface.glb', type: 'glb', opacity: 1,
    },
    solidInterface: {
        label: 'Solid interface', color: '#6750a4', group: new THREE.Group(), visible: true,
        file: 'billie-solid-interface.stl', type: 'stl', opacity: 1,
    },
};
Object.values(layers).forEach(layer => importRoot.add(layer.group));

const pelvicBody = new THREE.Group();
const pelvicAttach = new THREE.Group();
const overlayRoot = new THREE.Group();
planeRoot.add(pelvicBody, pelvicAttach, overlayRoot);
...
const planeState = { pick: [0, 1, 1], normal: [1, 0, 0] };
...
function frameObject(root, direction = new THREE.Vector3(-1.4, -1.8, 0.9)) {
    ...
    camera.position.copy(center).add(direction.normalize().multiplyScalar(radius * 3.8));
...
function buildInterfaceScene() {
    interfaceAnimal.add(layers.solidAnimal.group.clone(true));
    interfaceReferenceMechanical.add(layers.mechanical.group.clone(true));
    interfaceMechanical.add(layers.mechanical.group.clone(true));
    interfaceSolid.add(layers.solidInterface.group.clone(true));
...
function closestSurfacePoint(point, triangles = surfaceTriangles, bounds = interfaceAnimalBounds) {
    ...
    const fallback = planeData(interfaceBounds, interfacePlaneState, new THREE.Vector3(0, -1, 0)).normal;
    if (!bestTriangle) return { point: point.clone(), normal: fallback };
...
function applyPelvicStageVisibility() {
    ...
    Object.entries(planeLayerTargets).forEach(([key, target]) => {
        target.visible = planeLayerVisibility[key] && (key !== 'box' || activeStep === '2a');
    });
}
...
function showStep(step, shouldScroll = false) {
    ...
    renderLegend();
    const viewRoot = isImport ? importRoot : isPelvic ? planeRoot : activeStep === '3a' ? interfaceReferenceMechanical : interfaceRoot;
    ...
    if (shouldScroll) {
        ...
        if (window.matchMedia('(max-width: 880px)').matches) {
            ...
            document.getElementById('workflow-scroll').scrollTo({ top: section.offsetTop, behavior: 'smooth' });
        }
    }
}
...
document.querySelectorAll('[data-axis]').forEach(select => {
    select.addEventListener('change', () => {
        planeState.pick[+select.dataset.axis] = +select.value;
        updatePlane();
    });
});
...
const parameterValues = {
    socketThickness: Array.from({ length: 20 }, (_, index) => 4 + index * 0.5),
    boundaryThickness: Array.from({ length: 20 }, (_, index) => 4 + index * 0.5),
    pointCount: Array.from({ length: 20 }, (_, index) => 25 + index * 25),
    pelvicThickness: Array.from({ length: 20 }, (_, index) => 4 + index * 0.5),
    pelvicDistance: Array.from({ length: 20 }, (_, index) => 40 + index * 10),
};
document.querySelectorAll('[data-socket-param]').forEach(input => {
    const updateParameter = () => {
        ...
        input.setAttribute('aria-valuetext', value + ', model slot ' + (index + 1) + ' of 20');
        document.querySelector('[data-slot-status]').textContent = 'Model slot ' + (index + 1) + ' of 20 selected';
    };
...
document.getElementById('reset-view').addEventListener('click', () => {
    frameObject(activeStep === '1' ? importRoot : activeStep.startsWith('2') ? planeRoot : activeStep === '3a' ? interfaceReferenceMechanical : interfaceRoot);
});
...
Promise.all([
    ...
    loadGLB('billie-pelvic-body.glb').then(gltf => preparePelvic(gltf, pelvicBody, bodyLayer)),
    loadGLB('billie-pelvic-attach.glb').then(gltf => preparePelvic(gltf, pelvicAttach, attachLayer)),
]).then(() => {
